// 🔥 INÍCIO script.js ATUALIZADO COM SUAS CORREÇÕES 🔥
const canvas = document.getElementById("postCanvas");
const ctx = canvas.getContext("2d");

// ... [mantém tudo igual até a função drawPost]
...
async function drawPost({ tema, headline, subheadline, mensagem, legenda, tags, format, color }) {
  ...
  // BG Overlay (duas camadas - abaixo de tudo)
  try {
  ...
  try {
    ...
    ctx.drawImage(topRight, width - 85 - topRight.width, 93);

    // canto inferior esquerdo - ajuste específico para quadrado
    let arrowBottom = 143;
    if (format === "quadrado") arrowBottom = 100;

    ctx.drawImage(bottomLeft, 27, height - arrowBottom - bottomLeft.height);
  } catch (e) {
    console.warn("Erro decorativo:", e);
  }

  // Imagem principal - acima de tudo
  let imageBottomY = 0;
  try {
    if (!cachedImage) {
